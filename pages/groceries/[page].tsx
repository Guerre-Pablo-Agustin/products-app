...
function PaginatedPage({ products, currentPage, totalProducts }: PageProps) {
  return (
    <Page>
      <Head>
        <title>Groceries Page {currentPage} - SSG Pagination Example</title>
        <meta
          name="description"
...
export const getStaticProps: GetStaticProps = async ({
  ...
  const { products, total } = await getProducts({ limit: PER_PAGE, page , categories: ["groceries"] });
...
export const getStaticPaths: GetStaticPaths = async () => {
  return {
    paths: Array.from({ length: 5 }).map((_, i) => `/groceries/${i + 1}`),
    fallback: "blocking",
  };
};
